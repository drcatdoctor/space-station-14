...
namespace Content.Client.Atmos.Monitor.UI
{
    ...
    public sealed partial class AirAlarmWindow : DefaultWindow
    {
        ...
        public void UpdateDeviceData(string addr, IAtmosDeviceData device)
        {
            switch (device)
            {
                case VentOrScrubberData { DeviceType: AtmosDeviceType.Vent } data:
                    if (!_pumps.TryGetValue(addr, out var pumpControl))
                    {
                        var control= new PumpControl(data, addr);
                    ...
                    {
                        pumpControl.ChangeData(data);
                    }

                    break;
                case VentOrScrubberData { DeviceType: AtmosDeviceType.Scrubber } data:
